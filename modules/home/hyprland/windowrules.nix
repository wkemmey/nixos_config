...
let
  ...
  # Try to import host-specific window rules, fallback to default if doesn't exist
  hostWindowRulesPath = ./hosts/${host}/windowrules.nix;
  defaultWindowRulesPath = ./hosts/default/windowrules.nix;
  hostWindowRules =
    if builtins.pathExists hostWindowRulesPath then
      import hostWindowRulesPath { inherit host; }
    else
      import defaultWindowRulesPath { inherit host; };
